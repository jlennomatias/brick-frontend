        ...
        <q-form @submit.prevent="approveConsent">
            ...
            <div class="content-box">
                <div class="content-box-item">
                    <p>
                        Data de Pagamento: <span class="float-right bold">{{
                            confirmationData.consentData.consent.payment.date }}</span>
                    </p>
                </div>
                <br />
                ...
                <div class="content-box-item">
                    <p>Valor a pagar</p>
                    <p class="payment-amount">R$ {{ confirmationData.consentData.consent.payment.amount }}</p>
                </div>
            </div>
            <br />
            ...
            <div class="content-box">
                <div class="content-box-item">
                    <label>CPF:</label> <span>CPF {{ applyCPFMask(confirmationData.consentData.consent.creditor.cpfCnpj)
                        }}</span>
                </div>
                <div class="content-box-item">
                    <label>Nome:</label> {{ confirmationData.consentData.consent.creditor.name }}
                </div>
                <div class="content-box-item">
                    <label>Chave PIX:</label> <span>{{ confirmationData.consentData.consent.payment.details.proxy
                        }}</span>
                </div>
            </div>
            <br />
            ...
            <div v-for="(conta, index) in confirmationData.account" :key="index" class="content-box">
                ...
                <q-radio :id=index class="checkbox-inline confirmation-radio" v-model="selectedAccount" :val=index
                    size="28px" />
                <label class="label-inline lh-12 confirmation-label" :for=index>{{ `Ag ${conta.issuer} |
            ...
            <div class="content-box"><label class="lh-12">Tipo: </label>{{
                confirmationData.consentData.consent.payment.type }}</div>
            <br />
            <div class="open-finance-message">
                <p>
                    Para concluir o pagamento, lhe redirecionaremos devolta para a
                    instituição <span class="stronger-1">{{ confirmationData.consentData.consent.organizationName }}
                        <img src="https://www.w3.org/assets/logos/w3c/w3c-no-bars.svg" alt="Logo"
                            class="logo-img"></span>
                </p>
            </div>
            <div>
                <q-btn :label="loading ? '' : 'Concluir pagamento'" class="button-submit" no-caps type="submit">
                    <q-spinner v-if="loading" color="white" />
                </q-btn>
            </div>
            <div>
                <q-btn :label="loading ? '' : 'Cancelar'" class="empty-button" no-caps @click="cancelConsent">
                    <q-spinner v-if="loading" color="white" />
                </q-btn>
            </div>
        </q-form>
    ...
    methods: {
        getDeviceInfo() {
            ...
            if (navigator.userAgent.indexOf("Win") != -1) {
                ...
                os = "MacOS";
                osVersion = navigator.userAgent.match(/Mac OS X (\d+[_\.]\d+)/)[1].replace("_", ".");
            } else if (navigator.userAgent.indexOf("Linux") != -1) {
                os = "Linux";
                osVersion = navigator.userAgent.match(/Linux (\d+\.\d+)/) ? navigator.userAgent.match(/Linux (\d+\.\d+)/)[1] : "Unknown Version";
            } else if (navigator.userAgent.indexOf("Android") != -1) {
                os = "Android";
                osVersion = navigator.userAgent.match(/Android (\d+\.\d+)/)[1];
            } else if (navigator.userAgent.indexOf("like Mac") != -1) {
                os = "iOS";
                osVersion = navigator.userAgent.match(/CPU OS (\d+[_\.]\d+)/)[1].replace("_", ".");
            }
        ...
        async cancelConsent() {
            ...
            const respostaLogin = await axios.post(`/api/${process.env.CLIENT_ID}/consents/v1/interactions/${this.confirmationData.consentData.interactionId}/cancel`, body)
