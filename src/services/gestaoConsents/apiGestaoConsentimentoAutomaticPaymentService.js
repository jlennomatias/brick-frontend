...
export async function getConsentsAutomaticPayments(selectParams) {
  try {
    console.log('parametro: ', selectParams);

    const params = { "filter.status": selectParams };
    const response = await api.get(
      `/api/management/consents/v1/consents/automatic-payment/transmitions`,
      { params },
    );
...
export async function getConsentsAutomaticPaymentId(id, params) {
  try {
    const response = await api.get(
      `/api/management/consents/v1/consents/automatic-payment/transmitions/${id}`,
      params,
    );
    return response.data;
  } catch (error) {
